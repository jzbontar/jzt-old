local SpatialRandnPadding, parent = torch.class('jzt.SpatialRandnPadding', 'nn.Module')

function SpatialRandnPadding:__init(pad_l, pad_r, pad_t, pad_b)
   parent.__init(self)
   self.pad_l = pad_l
...
function SpatialRandnPadding:updateOutput(input)
   if input:dim() == 3 then
      -- sizes
...
function SpatialRandnPadding:updateGradInput(input, gradOutput)
   if input:dim() == 3 then 
